...
const card: React.CSSProperties = { background:'#fff', border:'1px solid rgba(7,13,31,.08)', borderRadius:12, padding:'10px 14px' }
const secLabel: React.CSSProperties = { fontSize:11, fontWeight:600, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:6, display:'block' }
...
export function BriefingPage({ workspace, brand, subscription, credits, navigate }: Props) {
  ...
  return (
    <div className="page-split">
      ...
      {/* ── ESQUERDA ── */}
      <div className="page-split-left" style={{gap:12}}>

        <div>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
            <span style={{color:'#F72585',fontSize:18}}>✦</span>
            <h1 style={{fontSize:18,fontWeight:700,color:'#070D1F',letterSpacing:'-.4px'}}>
        ...
        {phase==='plan' && (
          <>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                {([['semana','1 semana','~5 posts'],['quinzena','2 semanas','~10 posts'],['mes','1 mês','~20 posts']] as const).map(([v,l,s])=>(
                  <button key={v} onClick={()=>setPeriod(v)} style={{
                    padding:'8px 8px',textAlign:'center',cursor:'pointer',fontFamily:'inherit',
                    ...
                    <div style={{width:22,height:22,borderRadius:6,background:period===v?'rgba(247,37,133,.1)':'#F7F8FA',display:'flex',alignItems:'center',justifyContent:'center',color:period===v?'#F72585':'#9CA3AF'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                <div>
                  ...
                  <div style={{position:'relative'}}>
                    ...
                    <input type="date" value={startDate} onChange={e=>setStart(e.target.value)} style={{width:'100%',height:34,paddingLeft:32,paddingRight:10,border:'1px solid rgba(7,13,31,.1)',borderRadius:10,fontSize:13,fontFamily:'inherit',outline:'none',color:'#070D1F',background:'#F7F8FA'}} />
                  </div>
                </div>
                <div>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:10}}>
                {[
                  ...
                  <div key={row.label} style={{display:'flex',alignItems:'center'}}>
                    <div style={{width:28,height:28,borderRadius:8,background:row.bg,display:'flex',alignItems:'center',justifyContent:'center',color:row.color,marginRight:12,flexShrink:0}}>
                      {row.icon}
                    </div>
                    <div style={{flex:1}}>
            ...
            <div>
              ...
              <input value={theme} onChange={e=>setTheme(e.target.value)} placeholder="ex: coleção inverno, promoção, lançamento, dicas..." style={{width:'100%',height:36,padding:'0 14px',border:'1px solid rgba(7,13,31,.1)',borderRadius:10,fontSize:13,fontFamily:'inherit',outline:'none',background:'#fff',color:'#070D1F'}} />
            ...
            <button onClick={generate} disabled={loading||total===0} style={{width:'100%',height:44,background:loading||total===0?'#e5e7eb':'linear-gradient(135deg,#FF6A00 0%,#F72585 50%,#7B2CFF 100%)',border:'none',borderRadius:12,color:loading||total===0?'#9CA3AF':'white',fontSize:15,fontWeight:700,fontFamily:'inherit',cursor:loading||total===0?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,boxShadow:loading||total===0?'none':'0 4px 20px rgba(247,37,133,.35)',transition:'all .2s'}}>
              {loading?(
        ...
        {phase==='schedule' && (
          <>
            ...
            <div style={{flex:1}} />
            <button onClick={approveAll} disabled={approvingAll||credits<itemCr||items.every(i=>i.status!=='pending')} style={{width:'100%',height:44,background:'linear-gradient(135deg,#FF6A00,#F72585,#7B2CFF)',border:'none',borderRadius:12,color:'white',fontSize:15,fontWeight:700,fontFamily:'inherit',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,boxShadow:'0 4px 20px rgba(247,37,133,.35)',opacity:approvingAll||credits<itemCr||items.every(i=>i.status!=='pending')?0.5:1}}>
            ...
            <button onClick={()=>setPhase('plan')} style={{width:'100%',height:36,background:'transparent',border:'1px solid rgba(7,13,31,.12)',borderRadius:10,fontSize:13,cursor:'pointer',fontFamily:'inherit',color:'#374151'}}>← Rever configuração</button>
          </>
        )}
        ...
        {phase==='plan' && (
          <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 48px',gap:20}}>
            ...
            <div style={{textAlign:'center',maxWidth:320}}>
              <h2 style={{fontSize:18,fontWeight:700,color:'#070D1F',marginBottom:10,letterSpacing:'-.3px'}}>Seu cronograma vai aparecer aqui</h2>
...
function ItemCard({item,credits,onApprove,onUpdate,onAddRef,fileRef,onNavigate}:{
  ...
  return (
    <div style={{background:'#fff',borderRadius:12,overflow:'hidden',border:`1px solid ${item.status==='generating'?'rgba(247,37,133,.3)':'rgba(7,13,31,.08)'}`,boxShadow:'0 1px 3px rgba(7,13,31,.05)'}}>
      ...
      {item.editing&&(
        <div style={{padding:'14px 16px',background:'#F7F8FA',borderTop:'1px solid rgba(7,13,31,.07)',display:'flex',flexDirection:'column',gap:10}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div>
              <label style={{fontSize:11,fontWeight:600,color:'#9CA3AF',display:'block',marginBottom:5}}>Tema</label>
              <input style={{width:'100%',height:36,padding:'0 10px',border:'1px solid rgba(7,13,31,.1)',borderRadius:8,fontSize:12,fontFamily:'inherit',outline:'none'}} value={item.title} onChange={e=>onUpdate({title:e.target.value})} />
            </div>
            <div>
              <label style={{fontSize:11,fontWeight:600,color:'#9CA3AF',display:'block',marginBottom:5}}>Data</label>
              <input type="date" style={{width:'100%',height:36,padding:'0 10px',border:'1px solid rgba(7,13,31,.1)',borderRadius:8,fontSize:12,fontFamily:'inherit',outline:'none'}} value={item.scheduled_date} onChange={e=>onUpdate({scheduled_date:e.target.value})} />
            </div>
          </div>
          <div>
            <label style={{fontSize:11,fontWeight:600,color:'#9CA3AF',display:'block',marginBottom:5}}>Formato</label>
            <select style={{width:'100%',height:36,padding:'0 10px',border:'1px solid rgba(7,13,31,.1)',borderRadius:8,fontSize:12,fontFamily:'inherit',outline:'none',background:'#fff'}} value={item.content_type} onChange={e=>onUpdate({content_type:e.target.value as ContentType})}>
